...
void removeCycles(Edge **B, int *neb) {
  int *visiteds = malloc(sizeof(int)),
      sizeVisiteds = 0,
      newSizeB = 0,
      index, indexSrc, indexDest, auxIndex;
  Edge *newB = malloc(sizeof(Edge));

  for (index = 0; index < *neb; index++) {
    Edge edge = (*B)[index];
    auxIndex = -1;
    indexSrc = found(edge.src, visiteds, sizeVisiteds);
    indexDest = found(edge.dest, visiteds, sizeVisiteds);
    if (indexSrc == -1 || indexDest == -1) {
      newB[newSizeB] = edge;
      newSizeB++;
      newB = realloc(newB, (newSizeB + 1) * sizeof(Edge));
      addIfNotExists(&visiteds, &sizeVisiteds, edge.src);
    } else {
      if (newB[indexSrc].weight > edge.weight)
        newB[indexSrc] = edge;
    }
  }
...
void DFS(Graph *G, int startingV, int *numberOfVertex, Vertex **vertexConnected) {
  int i;
  Vertex *vertex = findVertex(G, startingV);
  // Mark the current node as visited
  vertex->visited = true;
  (*vertexConnected)[*numberOfVertex] = *vertex;
  *numberOfVertex += 1;
  *vertexConnected = realloc(*vertexConnected, (*numberOfVertex + 1) * sizeof(Vertex));

  // Recur for all the vertices adjacent
  // to this vertex
  Edge *edge = vertex->head;
  while (edge) {
    Vertex *vertexDest = findVertex(G, edge->dest);
  ...
  *nnc = 0;
  ...
  for (i = 0; i < GB->V; i++) {
    if (!GB->array[i].visited) {
      auxNv = 0;
      Vertex *arrayOfVertex = malloc(sizeof(Vertex));
...
Graph *populateNewGraph(Edge *A, int nva, int nea, Edge *B, int nvb, int neb, int nnc, Graph *F) {
  ...
  for (index = 0; index < nea; index++) {
    for (j = 0; j < nnc; j++) {
      if (found(A[index].src, vertexByComponent[j], sizeByComponent[j]) == -1) {
        addIfNotExists(&vertexInA, &sizeVertexInA, A[index].src);
      } else {
        A[index].src = j - nnc;
      }

      if (found(A[index].dest, vertexByComponent[j], sizeByComponent[j]) != -1) {
        A[index].dest = j - nnc;
      }
    }
  }
